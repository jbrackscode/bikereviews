...
const shopUrl = 'https://jbracks.com.au/collections/vertical-bike-racks?utm_source=bikereviews&utm_medium=lp&utm_campaign=families&utm_content=cta';
...
const reasons = [
  ...
  },
  {
    num: 7,
    flip: false,
    summary: "Family sizes for every family — 4, 5, or 6 bikes RRP $950",
...
const fbComments = [
  ...
  },
  {
    ...
    name: "Michael T.",
    time: "3d",
    text: "Bought the 6-bike rack for our crew — 2 adults, 3 kids, and sometimes my dad's e-bike. It handles all of it without breaking a sweat. The slow-fold strut is genuinely excellent, especially when you've got kids running around near the car. Worth every cent.",
    likes: "👍 98",
    liked: false,
    color: "#3a9e6e",
  },
  {
    ...
    name: "Jess R.",
    time: "2d",
    text: "I was comparing JB Racks to VelociRAX and Dirty Possum — went with JB Racks because of the swing arm and the 4-year warranty. So glad I did. Customer service is also excellent. When I had a question about fitting it to our Fortuner they responded within an hour.",
    likes: "👍 76",
    liked: false,
    color: "#b55a3a",
  },
  {
    ...
    name: "Dave W.",
    time: "1d",
    text: "Three road trips to the South West now. The rack has been flawless. We do about 3–4 hours of highway driving each way and the bikes don't move a centimetre. Wish I'd done this years ago instead of faffing with roof racks.",
    likes: "👍 54",
    liked: false,
    color: "#7a5ab5",
  },
  {
    ...
    name: "Sarah H.",
    time: "22h",
    text: "The kids call it 'the bike tower' which I think is the highest praise possible from an 8-year-old 😂 Genuinely great product, shipping was fast, and the instructions were clear. Already recommended it to two other school families.",
    likes: "❤️😍 211",
    liked: true,
    color: "#b5894a",
  },
  ...
  return (
    <div className="max-w-[1140px] mx-auto" style={{ fontFamily: "'Libre Franklin', system-ui, sans-serif" }}>
      ...
      <div style={{ background: 'var(--white)', padding: '44px 40px 36px', borderBottom: '3px solid var(--brand)' }}>
        ...
        >
          7 Reasons Why Vertical Bike Racks Are{' '}
          <em style={{ fontStyle: 'italic', color: 'var(--brand)' }}>Every Family&apos;s</em> Secret Weapon for Weekend
          Rides
        </h1>
        ...
        >
          <strong style={{ color: 'var(--ink)' }}>Summary:</strong> We discovered why families across Australia are
          switching from platform racks — and why once you go vertical, you&apos;ll never go back. After three years of
          wrestling with a roof rack and losing car boots to platform racks, we found the solution that changed our
          weekends completely.
        </div>
      </div>
        ...
        {reasons.map((r) => (
          <div key={r.num} style={{ padding: '48px 0', borderTop: '1px solid var(--rule)' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center mt-5">
              {/* Image — on flip sections, pushed to right col on desktop */}
              ...
              <div>
                ...
                {r.paragraphs.map((p, i) => (
                  <p
                    key={i}
                    style={{ fontSize: '16px', color: 'var(--ink-mid)', lineHeight: 1.75, marginBottom: '12px' }}
                  >
                    {p}
                  </p>
                ))}
                ...
                <div className="flex flex-wrap gap-x-5 gap-y-2.5 mt-4">
                  {r.pills.map((pill) => (
                    <span
                      ...
                      style={{ fontSize: '14px', fontWeight: 500, color: 'var(--ink-mid)' }}
                    >
                      <span
      ...
      <div style={{ background: 'var(--bg)', padding: '48px 40px', borderTop: '1px solid var(--rule)' }}>
        <h2
          ...
          style={{ fontFamily: "'Libre Franklin', system-ui, sans-serif", fontSize: '28px', fontWeight: 900, color: 'var(--ink)' }}
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {reviews.map((r) => (
            ...
            >
              <div style={{ color: 'var(--brand)', fontSize: '15px', marginBottom: '10px', letterSpacing: '2px' }}>
                ★★★★★
              </div>
              <blockquote
                style={{
              ...
              <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--ink)' }}>
                {r.author}{' '}
                <span style={{ fontWeight: 400, color: 'var(--ink-muted)' }}>· Verified Buyer</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ background: 'var(--white)', padding: '0 40px 48px' }}>
        <div className="grid grid-cols-3 gap-2.5">
          {ugcPhotos.map((photo) => (
            <div
              key={photo.src}
              style={{ aspectRatio: '1', borderRadius: '8px', overflow: 'hidden', background: 'var(--bg)' }}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={photo.src}
      ...
      <div style={{ background: 'var(--bg)', padding: '0 40px 48px' }}>
        ...
        >
            ...
            <div className="flex items-start gap-2.5 mb-2.5">
              ...
              <div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: '#050505' }}>
                  <span style={{ color: '#1877f2' }}>JB Racks Australia</span>
                </div>
            ...
            <div className="flex" style={{ paddingBottom: '2px' }}>
              {['👍 Like', '💬 Comment', '↗ Share'].map((btn) => (
                <div
                  key={btn}
                  className="flex items-center gap-1.5"
                  style={{ padding: '6px 12px', fontSize: '14px', fontWeight: 600, color: '#65676b', cursor: 'pointer' }}
                >
                  {btn}
                </div>
              ))}
          ...
          <div style={{ padding: '10px 14px 14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div key={c.initials} className="flex gap-2 items-start">
                ...
                <div className="flex flex-col gap-0.5 flex-1">
                  <div style={{ background: '#f0f2f5', borderRadius: '18px', padding: '8px 14px' }}>
                    <div style={{ fontSize: '13px', fontWeight: 700, color: '#050505', marginBottom: '2px' }}>
                      {c.name}
                    </div>
                  ...
                  <div className="flex items-center gap-2.5 pl-2">
                    ...
                    <span style={{ fontSize: '12px', color: '#65676b', marginLeft: 'auto', paddingRight: '4px' }}>
                      {c.likes}
                    </span>
                  </div>
                </div>
            ...
            <div
              style={{ fontSize: '13px', fontWeight: 700, color: '#65676b', cursor: 'pointer', padding: '4px 8px', marginTop: '4px' }}
            >
              View 209 more comments...
            </div>
      ...
      {/* DEAL STRIP */}
      <div
        className="flex flex-col md:flex-row justify-center gap-2 md:gap-8 text-center"
        style={{
          background: 'var(--ink)',
          padding: '8px 40px',
          fontSize: '13px',
